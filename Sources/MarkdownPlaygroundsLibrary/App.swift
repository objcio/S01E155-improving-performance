...
final class ViewController: NSViewController, NSTextStorageDelegate {
    let editor = NSTextView()
    let output = NSTextView()
    var observerToken: Any?
    var codeBlocks: [CodeBlock] = []
    var repl: REPL!
    var invalidRange: NSRange = NSRange()
    ...
    override func viewDidLoad() {
        ...
        observerToken = NotificationCenter.default.addObserver(forName: NSTextView.didChangeNotification, object: editor, queue: nil) { [unowned self] _ in
            self.parse(range: self.invalidRange)
        }
        self.parse()
    }
    
    func textStorage(_ textStorage: NSTextStorage, willProcessEditing editedMask: NSTextStorageEditActions, range editedRange: NSRange, changeInLength delta: Int) {
        invalidRange = editedRange
    }
    
    func parse(range: NSRange? = nil) {
        guard let attributedString = editor.textStorage else { return }
        let invalidRange = range ?? attributedString.range
        codeBlocks = attributedString.highlightMarkdown(invalid: invalidRange)
    }
