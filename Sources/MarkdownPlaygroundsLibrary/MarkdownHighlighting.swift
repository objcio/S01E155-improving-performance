...
extension NSMutableAttributedString {
    ...
    func highlightMarkdown(invalid: NSRange) -> [CodeBlock] {
        ...
        func range(of node: Node) -> NSRange? {
            guard node.start.line != 0 else { return nil } // CommonMark returns (0, 0) position for soft breaks
        ...
        let invalidBlocks = node.children.filter { child in
            guard let r = range(of: child) else { return false }
            return invalid.intersection(r) != nil
        }
        
        let invalidRange: NSRange
        if invalidBlocks.isEmpty {
            invalidRange = NSRange()
        } else {
            let start = range(of: invalidBlocks[0])!.location
            let end = range(of: invalidBlocks.last!)!.upperBound
            invalidRange = NSRange(location: start, length: end-start)
        }
        
        let defaultAttributes = Attributes(family: "Helvetica", size: 16)
        setAttributes(defaultAttributes.atts, range: invalidRange)
        
        for block in invalidBlocks {
            block.visitAll(defaultAttributes) { c, attributes in
                guard let nsRange = range(of: c) else { return }
                switch c.type {
                ...
                case CMARK_NODE_CODE_BLOCK:
                    ...
                    if let tokens = try? code.highlightSwift() {
                        let codeString = (string as NSString).substring(with: nsRange) as NSString
                        let offset = nsRange.location + codeString.range(of: code).location
                        for token in tokens {
...
extension CommonMark.Node {
    ...
    func visitAll<State>(_ initial: State, _ callback: (Node, inout State) -> ()) {
        ...
        callback(self, &initialState)
        for c in children {
            var copy = initialState
            callback(c, &copy)
